// This is LokiJS (https://github.com/techfort/LokiJS), updated to modern JS
// standards and edited for Deno compatibility by Adam Nelson.
//
// Original code is MIT licensed, (c) 2015 Joe Minichino
...
const Utils = {
  ...
  resolveTransformObject(subObj, params, depth) {
    let prop,
      pname;
    ...
    for (prop in subObj) {
      if (
        ...
        if (hasOwnProperty.call(params, pname)) {
  ...
  resolveTransformParams(transform, params) {
    let idx,
      clonedStep,
      resolvedTransform = [];
  ...
  // getIn({a: {b: 1}}, "a.b", true) => 1
  getIn(object, path, usingDotNotation) {
    if (object == null) {
      return undefined;
    }
    if (!usingDotNotation) {
...
function aeqHelper(prop1, prop2) {
  ...
  if (
    !prop1 || !prop2 || prop1 === true || prop2 === true || prop1 !== prop1 ||
    prop2 !== prop2
  ) {
...
let LokiOps = {
  ...
  $between(a, vals) {
    ...
    return (Comparators.gt(a, vals[0], true) &&
...
class LokiEventEmitter {
   ...
   * @memberof LokiEventEmitter
   */
  addListener = LokiEventEmitter.prototype.on;
 ...
 */
class Loki extends LokiEventEmitter {
  /**
  ...
  constructor(filename, options) {
    ...
    this.verbose = options && hasOwnProperty.call(options, "verbose")
      ? options.verbose
      : false;
    ...
    let getENV = () => {
      if (
        typeof global !== "undefined" && (global.android || global.NSObject)
      ) {
    ...
    if (options && hasOwnProperty.call(options, "env")) {
   ...
   * @param {function} options.autosaveCallback
  ...
  configureOptions(options, initialConfig) {
    ...
    if (typeof (options) !== "undefined") {
      ...
      if (hasOwnProperty.call(this.options, "persistenceMethod")) {
        ...
        if (
          ...
          this.persistenceAdapter =
            new persistenceMethods[options.persistenceMethod]();
        }
        // should be throw an error here, or just fall back to defaults ??
      }

      // if user passes adapter, set persistence mode to adapter and retain persistence adapter instance
      if (hasOwnProperty.call(this.options, "adapter")) {
      ...
      if (hasOwnProperty.call(this.options, "autosaveInterval")) {
        this.autosaveDisable();
        this.autosaveInterval = parseInt(this.options.autosaveInterval, 10);
      }

      if (
        hasOwnProperty.call(this.options, "autosave") && this.options.autosave
      ) {
        this.autosaveDisable();
        this.autosave = true;

        if (hasOwnProperty.call(this.options, "autosaveCallback")) {
      ...
      if (hasOwnProperty.call(this.options, "throttledSaves")) {
        this.throttledSaves = this.options.throttledSaves;
      }
    } // end of options processing

    // ensure defaults exists for options which were not set
    if (!hasOwnProperty.call(this.options, "serializationMethod")) {
      this.options.serializationMethod = "normal";
    }

    // ensure passed or default option exists
    if (!hasOwnProperty.call(this.options, "destructureDelimiter")) {
    ...
    if (this.persistenceAdapter === null) {
      ...
      if (this.persistenceMethod) {
        this.persistenceAdapter =
          new persistenceMethods[this.persistenceMethod]();
      }
    }
  }
  ...
  copy(options) {
    ...
    let databaseCopy = new Loki(this.filename, { env: "NA" });
    let clen, idx;
    ...
    if (
      hasOwnProperty.call(options, "removeNonSerializable") &&
  ...
  addCollection(name, options) {
    ...
    let collection = new Collection(name, options);
  ...
  renameCollection(oldName, newName) {
    let c = this.getCollection(oldName);
  ...
  removeCollection(collectionName) {
    ...
    for (i = 0; i < len; i += 1) {
      if (this.collections[i].name === collectionName) {
        let tmpcol = new Collection(collectionName, {});
        let curcol = this.collections[i];
        for (let prop in curcol) {
          if (
            hasOwnProperty.call(curcol, prop) &&
            hasOwnProperty.call(tmpcol, prop)
          ) {
  ...
  serialize(options) {
    ...
    if (!hasOwnProperty.call(options, "serializationMethod")) {
    ...
    switch (options.serializationMethod) {
      ...
      case "destructured": // use default options
  ...
  serializeDestructured(options) {
    ...
    if (!hasOwnProperty.call(options, "partitioned")) {
      options.partitioned = false;
    }

    if (!hasOwnProperty.call(options, "delimited")) {
      options.delimited = true;
    }

    if (!hasOwnProperty.call(options, "delimiter")) {
    ...
    if (
      ...
      hasOwnProperty.call(options, "partition") &&
      options.partition >= 0
    ) {
  ...
  serializeCollection(options) {
    ...
    if (!hasOwnProperty.call(options, "collectionIndex")) {
      throw new Error(
  ...
  deserializeDestructured(
    ...
    let workarray = [];
    let len, cdb;
    let collIndex = 0, collCount, lineIndex = 1, done = false;
    let currObject;
    ...
    if (options.partitioned) {
      ...
      if (hasOwnProperty.call(options, "partition")) {
        // db only
  ...
  loadJSONObject(dbObject, options) {
    ...
    if (
      hasOwnProperty.call(dbObject, "throttledSaves") && options &&
      !hasOwnProperty.call(options, "throttledSaves")
    ) {
    ...
    for (i; i < len; i += 1) {
      ...
      copyColl = this.addCollection(coll.name, {
        ...
        disableFreeze: hasOwnProperty.call(coll, "disableFreeze")
          ? coll.disableFreeze
          : true,
      });

      copyColl.adaptiveBinaryIndices =
        hasOwnProperty.call(coll, "adaptiveBinaryIndices")
      ...
      if (coll.getData) {
        if (
          (options && hasOwnProperty.call(options, coll.name)) ||
          !copyColl.disableFreeze || copyColl.autoupdate
        ) {
          throw new Error(
        ...
        j = 0;
        if (options && hasOwnProperty.call(options, coll.name)) {
      ...
      if (hasOwnProperty.call(coll, "uniqueNames")) {
  ...
  generateChangesNotification(arrayOfCollectionNames) {
    ...
    let changes = [],
      selectedCollections = arrayOfCollectionNames ||
  ...
  throttledSaveDrain(callback, options) {
    ...
    options = options || {};
    if (!hasOwnProperty.call(options, "recursiveWait")) {
      options.recursiveWait = true;
    }
    if (!hasOwnProperty.call(options, "recursiveWaitLimit")) {
      options.recursiveWaitLimit = false;
    }
    if (!hasOwnProperty.call(options, "recursiveWaitLimitDuration")) {
      options.recursiveWaitLimitDuration = 2000;
    }
    if (!hasOwnProperty.call(options, "started")) {
      ...
      if (options.recursiveWait) {
        ...
        this.throttledCallbacks.push(() => {
          ...
          if (this.throttledSavePending) {
            ...
            this.throttledSaveDrain(callback, options);
            return;
        ...
        (dbString) => {
          if (typeof (dbString) === "string") {
            ...
            if (!dbString) {
              ...
              this.emit(
                "loaded",
                "empty database " + this.filename + " loaded",
              );
              return;
            }
  ...
  loadDatabase(options, callback) {
    ...
    this.throttledSaveDrain((success) => {
      if (success) {
        ...
        this.loadDatabaseInternal(options, (err) => {
    ...
    if (this.persistenceAdapter.mode === "incremental") {
      ...
      this.persistenceAdapter.saveDatabase(
        ...
        () => {
          ...
          let lokiCopy = this.copy({ removeNonSerializable: true });
  ...
  saveDatabase(callback) {
    ...
    let localCallbacks = this.throttledCallbacks;
    this.throttledCallbacks = [];
   ...
   *
   * @param {object} _options - not currently used (remove or allow overrides?)
   * @param {function=} callback - (Optional) user supplied async callback
   */
  autosaveEnable(_options, callback) {
    this.autosave = true;
...
class LokiMemoryAdapter {
  ...
  constructor(options) {
    ...
    if (!hasOwnProperty.call(this.options, "asyncResponses")) {
      this.options.asyncResponses = false;
    }

    if (!hasOwnProperty.call(this.options, "asyncTimeout")) {
  ...
  loadDatabase(dbname, callback) {
    if (this.options.asyncResponses) {
      setTimeout(() => {
        if (hasOwnProperty.call(this.hashStore, dbname)) {
  ...
  saveDatabase(
    ...
    if (this.options.asyncResponses) {
      setTimeout(() => {
        saveCount = hasOwnProperty.call(this.hashStore, dbname)
...
class LokiPartitioningAdapter {
  ...
  constructor(adapter, options) {
    ...
    if (!hasOwnProperty.call(this.options, "paging")) {
      this.options.paging = false;
    }

    // default to page size of 25 megs (can be up to your largest serialized object size larger than this)
    if (!hasOwnProperty.call(this.options, "pageSize")) {
      this.options.pageSize = 25 * 1024 * 1024;
    }

    if (!hasOwnProperty.call(this.options, "delimiter")) {
  ...
  loadDatabase(dbname, callback) {
    ...
    this.adapter.loadDatabase(dbname, (result) => {
   ...
   * @param {function} callback - adapter callback to return load result to caller
   */
  loadNextPartition(partition, callback) {
    let keyname = this.dbname + "." + partition;
    ...
    this.adapter.loadDatabase(keyname, (result) => {
      let data = this.dbref.deserializeCollection(result, {
        delimited: true,
  ...
  loadNextPage(callback) {
    ...
    let keyname = this.dbname + "." + this.pageIterator.collection + "." +
    ...
    this.adapter.loadDatabase(keyname, (result) => {
      ...
      for (idx = 0; idx < dlen; idx++) {
        this.dbref.collections[this.pageIterator.collection].data.push(
          JSON.parse(data[idx]),
        );
        data[idx] = null;
      }
  ...
  exportDatabase(dbname, dbref, callback) {
    let idx, clen = dbref.collections.length;
   ...
   * @param {function} callback - adapter callback to return load result to caller
   */
  saveNextPartition(callback) {
    let partition = this.dirtyPartitions.shift();
    let keyname = this.dbname + ((partition === -1) ? "" : ("." + partition));
    ...
    let result = this.dbref.serializeDestructured({
      partitioned: true,
      delimited: true,
      partition: partition,
    });

    this.adapter.saveDatabase(keyname, result, (err) => {
      if (err) {
        callback(err);
        return;
      }
   ...
   * @param {function} callback - adapter callback to return load result to caller
   */
  saveNextPage(callback) {
    let coll = this.dbref.collections[this.pageIterator.collection];
    let keyname = this.dbname + "." + this.pageIterator.collection + "." +
      this.pageIterator.pageIndex;
    let pageLen = 0,
...
class Resultset {
   ...
   * let result = users.chain().simplesort("age", true).limit(2).data();
   */
  limit(qty) {
    ...
    let rscopy = new Resultset(this.collection);
   ...
   * let result = users.chain().simplesort("age", true).offset(2).data();
   */
  offset(pos) {
    // if this has no filters applied, we need to populate filteredrows first
    if (!this.filterInitialized && this.filteredrows.length === 0) {
   ...
   *   }
   * ]);
   * let results = users.chain().transform("CountryFilter", { Country: 'fr' }).data();
   */
  transform(transform, parameters) {
    ...
    if (typeof transform === "string") {
      if (hasOwnProperty.call(this.collection.transforms, transform)) {
  ...
  sort(comparefun) {
    ...
    let wrappedComparer = (function (userComparer, data) {
      return function (a, b) {
   ...
   * @memberof Resultset
   * @example
   * let results = users.chain().simplesort('age').data();
   */
  simplesort(propname, options) {
    let eff,
      ...
      hasBinaryIndex = hasOwnProperty.call(
        this.collection
          .binaryIndices,
        propname,
      );
    ...
    if (frl === 0) {
      ...
      if (hasOwnProperty.call(this.collection.binaryIndices, propname)) {
        // make sure index is up-to-date
        this.collection.ensureIndex(propname);
        // copy index values into filteredrows
        this.filteredrows = this.collection.binaryIndices[propname].values
          .slice(
            0,
          );
      ...
      if (!options.disableIndexIntersect && hasBinaryIndex) {
        ...
        if (eff <= targetEff || options.forceIndexIntersect) {
          ...
          let pv = this.collection.binaryIndices[propname].values;
    ...
    const wrappedComparer = (function (prop, desc, data) {
      let val1, val2, arr;
      return function (a, b) {
        if (~prop.indexOf(".")) {
          arr = prop.split(".");
  ...
  compoundsort(properties) {
    ...
    const wrappedComparer = (function (props, data) {
      return function (a, b) {
   ...
   * @memberof Resultset
   * @example
   * let over30 = users.chain().find({ age: { $gte: 30 } }).data();
   */
  find(query, firstOnly) {
    ...
    if (
      doIndexCheck && this.collection.binaryIndices[property] &&
      indexedOps[operator]
    ) {
    ...
    if (
      !searchByIndex && operator === "$in" && Array.isArray(value) &&
   ...
   * @memberof Resultset
   * @example
   * let over30 = users.chain().where(function(obj) { return obj.age >= 30; }.data();
   */
  where(fun) {
    let viewFunction,
      result = [];
   ...
   * @memberof Resultset
   * @example
   * let over30Count = users.chain().find({ age: { $gte: 30 } }).count();
   */
  count() {
    if (this.filterInitialized) {
   ...
   * @memberof Resultset
   * @example
   * let resutls = users.chain().find({ age: 34 }).data();
   */
  data(options) {
    let result = [],
   ...
   * @memberof Resultset
   * @example
   * let db = new loki("order.db");
   * let orders = db.addCollection("orders");
   ...
   *   return grandTotal;
   * }
   * let grandOrderTotal = orders.chain().mapReduce(mapfun, reducefun);
   ...
   *
   * let mapfun = function(left, right) {
   *   return {
   *     orderId: left.$loki,
   ...
   * let orderSummary = orders.chain().eqJoin(products, "prodId", "productId", mapfun).data();
   *
   ...
   * @memberof Resultset
   * @example
   * let orders.chain().find({ productId: 32 }).map(function(obj) {
   *   return {
   *     orderId: $loki,
   *     productId: productId,
   *     quantity: qty
 ...
 */
class DynamicView extends LokiEventEmitter {
  /**
  ...
  constructor(collection, name, options) {
    ...
    if (!hasOwnProperty.call(this.options, "persistent")) {
    ...
    if (!hasOwnProperty.call(this.options, "sortPriority")) {
      this.options.sortPriority = "passive";
    }

    if (!hasOwnProperty.call(this.options, "minRebuildInterval")) {
  ...
  rematerialize(options) {
    ...
    if (hasOwnProperty.call(options, "removeWhereFilters")) {
   ...
   * let coll = db.addCollection('mydocs');
   * let dv = coll.addDynamicView('myview');
   * let tx = [
   *   {
   *     type: 'offset',
   ...
   * // add some records
   *
   * let results = dv.branchResultset('viewPaging', { pageStart: 10, pageSize: 10 }).data();
   */
  ...
  removeDocument(objIndex) {
    ...
    if (Object.keys(fxo).length > 0) {
      ...
      if (this.options.persistent) {
        ...
        this.resultdata = this.resultdata.filter(function (_obj, idx) {
          return !fxo[idx];
        });
      }
...
class Collection extends LokiEventEmitter {
  ...
  constructor(name, options) {
    ...
    if (hasOwnProperty.call(options, "unique")) {
    ...
    if (hasOwnProperty.call(options, "exact")) {
    ...
    this.adaptiveBinaryIndices =
      hasOwnProperty.call(options, "adaptiveBinaryIndices")
        ? options.adaptiveBinaryIndices
        : true;

    // is collection transactional
    this.transactional = hasOwnProperty.call(options, "transactional")
      ? options.transactional
      : false;

    // options to clone objects when inserting them
    this.cloneObjects = hasOwnProperty.call(options, "clone")
      ? options.clone
      : false;

    // default clone method (if enabled) is parse-stringify
    this.cloneMethod = hasOwnProperty.call(options, "cloneMethod")
      ? options.cloneMethod
      : "parse-stringify";

    // option to make event listeners async, default is sync
    this.asyncListeners = hasOwnProperty.call(options, "asyncListeners")
      ? options.asyncListeners
      : false;

    // if set to true we will not maintain a meta property for a document
    this.disableMeta = hasOwnProperty.call(options, "disableMeta")
      ? options.disableMeta
      : false;

    // disable track changes
    this.disableChangesApi = hasOwnProperty.call(options, "disableChangesApi")
      ? options.disableChangesApi
      : true;

    // disable delta update object style on changes
    this.disableDeltaChangesApi =
      hasOwnProperty.call(options, "disableDeltaChangesApi")
        ? options.disableDeltaChangesApi
        : true;
    if (this.disableChangesApi) this.disableDeltaChangesApi = true;

    // option to observe objects and update them automatically, ignored if Object.observe is not supported
    this.autoupdate = hasOwnProperty.call(options, "autoupdate")
      ? options.autoupdate
      : false;
    ...
    this.serializableIndices =
      hasOwnProperty.call(options, "serializableIndices")
        ? options.serializableIndices
        : true;

    // option to deep freeze all documents
    this.disableFreeze = hasOwnProperty.call(options, "disableFreeze")
    ...
    const getObjectDelta = (oldObject, newObject) => {
      const propertyNames = newObject !== null && typeof newObject === "object"
      ...
      if (
        ...
        for (let i = 0; i < propertyNames.length; i++) {
          ...
          if (hasOwnProperty.call(newObject, propertyName)) {
            if (
              !hasOwnProperty.call(oldObject, propertyName) ||
              this.uniqueNames.indexOf(propertyName) >= 0 ||
    ...
    this.on("delete", (obj) => {
      if (!this.disableChangesApi) {
        this.createChange(this.name, "R", obj);
      }
    });

    this.on("warning", (warning) => {
      this.lokiConsoleWrapper.warn(warning);
    });
  ...
  /*
    ...
    */
  createChange(name, op, obj, old) {
    this.changes.push({
      name: name,
      operation: op,
      obj: op == "U" && !this.disableDeltaChangesApi
  ...
  insertMeta(obj) {
    ...
    if (Array.isArray(obj)) {
      ...
      for (idx = 0; idx < len; idx++) {
        if (!hasOwnProperty.call(obj[idx], "meta")) {
          obj[idx].meta = {};
        }
  ...
  updateMetaWithChange(obj, old, objFrozen) {
  ...
  addTransform(name, transform) {
    if (hasOwnProperty.call(this.transforms, name)) {
  ...
  byExample(template) {
    ...
    query = [];
    for (k in template) {
      if (!hasOwnProperty.call(template, k)) continue;
      query.push(
        (
          obj = {}, obj[k] = template[k], obj
        ),
      );
    }
    return {
  ...
  ttlDaemonFuncGen() {
    ...
    return () => {
      const now = Date.now();
      const toRemove = this.chain().where(function daemonFilter(member) {
  ...
  configureOptions(options) {
    ...
    if (hasOwnProperty.call(options, "adaptiveBinaryIndices")) {
  ...
  ensureIndex(property, force) {
    ...
    if (
      ...
      hasOwnProperty.call(this.binaryIndices, property) && !force
    ) {
      return;
    }
    ...
    const wrappedComparer = (function (prop, data) {
      let val1, val2;
   ...
   * let result = coll.checkAllIndexes({ repair: true, randomSampling: true, randomSamplingFactor: 0.15 });
   * if (result.length > 0) {
  ...
  checkAllIndexes(options) {
    let key, bIndices = this.binaryIndices;
    let results = [], result;
   ...
   * @example
   * // full test
   * let valid = coll.checkIndex('name');
  ...
  checkIndex(property, options) {
    ...
    let valid = true, idx, iter, pos, len, biv;

    // make sure we are passed a valid binary index name
    if (!hasOwnProperty.call(this.binaryIndices, property)) {
      throw new Error(
  ...
  getBinaryIndexValues(property) {
    let idx, idxvals = this.binaryIndices[property].values;
    const result = [];
   ...
   */
  insert(doc, overrideAdaptiveIndices) {
   ...
   */
  update(doc) {
    let adaptiveBatchOverride, k, len;
    ...
    try {
      ...
      this.uniqueNames.forEach((key) => {
        this.getUniqueIndex(key, true).update(oldInternal, newInternal);
      });
  ...
  adaptiveBinaryIndexInsert(
    ...
    const usingDotNotation = (binaryIndexName.indexOf(".") !== -1);
    const index = this.binaryIndices[binaryIndexName].values;
    let val = Utils.getIn(
  ...
  adaptiveBinaryIndexRemove(
    ...
    let len, idx, rmidx, rmlen, rxo = {};
    let curr, shift, idxPos;
  ...
  calculateRange(op, prop, val) {
    ...
    const usingDotNotation = (prop.indexOf(".") !== -1);
  ...
  extractNumerical(field) {
    return this.extract(field).map(parseBase10).filter(Number).filter(
      function (n) {
        return !(isNaN(n));
      },
    );
